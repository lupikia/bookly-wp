 ...
 * Form Integration
 */
class pebas_pl_submit_listing_form {
	...
	public static function append_package_name( $title ) {
		if ( ! empty( $_POST ) && ! is_admin() && is_main_query() && in_the_loop() && is_page( get_option( 'job_manager_submit_job_form_page_id' ) ) && self::$package_id && 'before' === get_option( 'job_manager_paid_listings_flow' ) && apply_filters( 'wcpl_append_package_name', true ) ) {
			if ( self::$is_user_package ) {
				$package = pebas_pl_get_user_package( self::$package_id );
	...
	public static function scripts() {
		if ( is_page( get_option( 'job_manager_submit_job_form_page_id' ) ) ) {
			wp_enqueue_script( 'pebas-pl-theme', PEBAS_PL_URL . 'assets/scripts/pl-theme.js', array( 'jquery' ), '1.0.0', true );
		}
	}
	...
	public static function submit_button_text() {
		return __( 'Choose a package &rarr;', 'pebas-paid-listings' );
	}
	...
	public static function get_packages( $post__in = array() ) {
		return get_posts( apply_filters( 'pebas_paid_listings_get_job_packages_args', array(
			'post_type'        => 'product',
			'posts_per_page'   => - 1,
			'post__in'         => $post__in,
			'order'            => 'asc',
	...
	public static function submit_job_steps( $steps ) {
		if ( self::get_packages() && apply_filters( 'ppl_enable_paid_listing_submission', true ) ) {
			...
			$steps['wc-choose-package'] = array(
				'name'     => __( 'Choose a package', 'pebas-paid-listings' ),
	...
	public static function choose_package( $atts = array() ) {
		...
		$user_packages = pebas_pl_get_user_packages( get_current_user_id(), 'job_listing' );
		$button_text   = 'before' !== get_option( 'job_manager_paid_listings_flow' ) ? __( 'Submit &rarr;', 'pebas-paid-listings' ) : __( 'Listing Details &rarr;', 'pebas-paid-listings' );
		?>
		<?php include lisner_helper::get_template_part( 'header-media', 'pages/header' ); ?>
                ...
                <div class="row row-wrapper">
                    <div class="col-sm-12">
                        ...
                        <section class="main-content">
                            <div class="row">
                                <form method="post" id="listing_packages_form">
	...
	private static function validate_package( $package_id, $is_user_package ) {
		if ( empty( $package_id ) ) {
			return new WP_Error( 'error', __( 'Invalid Package', 'pebas-paid-listings' ) );
		} elseif ( $is_user_package ) {
			if ( ! pebas_pl_package_is_valid( get_current_user_id(), $package_id ) ) {
				return new WP_Error( 'error', __( 'Invalid Package', 'pebas-paid-listings' ) );
			}
		} else {
			...
			if ( class_exists( 'WC_Subscriptions' )
				...
				if ( wcs_user_has_subscription( get_current_user_id(), $package_id, 'active' ) ) {
					return new WP_Error( 'error', __( 'You already have this subscription.', 'pebas-paid-listings' ) );
				}
			}
		}
	...
	private static function process_package( $package_id, $is_user_package, $job_id ) {
		...
		if ( $is_user_package ) {
			...
			if ( in_array( get_post_status( $job_id ), array( 'pending_payment', 'expired' ) ) ) {
				pebas_pl_approve_job_listing_with_package( $job_id, get_current_user_id(), $package_id );
			}

			do_action( 'pebas_paid_listings_process_package_for_job_listing', $package_id, $is_user_package, $job_id );
	...
	/**
	 * Choose listing package handler
	 *
	 * @return bool
	 * @throws Exception
	 */
...
pebas_pl_submit_listing_form::init();
