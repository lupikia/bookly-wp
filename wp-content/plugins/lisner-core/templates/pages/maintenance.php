				...
				<div class="col-sm-12 text-center">
					...
					<?php if ( isset( $chimp ) ) : ?>
						<div class="maintenance-chimp">
							<p class="mailchimp-notice"><?php esc_html_e( 'Would you like to stay involved?', 'lisner-core' ) ?></p>
							<form class="newsletter-form" method="post">
								<div class="input-group">
									<label for="mailchimp_email">
										<i class="material-icons mf"><?php echo esc_html( 'email' ); ?></i>
										<?php esc_html_e( 'Email:', 'lisner-core' ); ?>
									</label>
									<input id="mailchimp_email" type="email" name="email"
									       placeholder="<?php esc_attr_e( 'Your valid email address', 'lisner-core' ); ?>">
								</div>
								<button type="submit" class="btn btn-secondary newsletter-submit"><i
											class="material-icons mf"><?php echo esc_html( 'send' ); ?></i></button>
								<input type="hidden" name="action" value="newsletter_ajax">
								<input type="hidden" name="mailchimp_api" value="<?php echo esc_attr( $chimp ); ?>">
					...
					<?php
					$facebook = isset( $option['maintenance-facebook'] ) ? $option['maintenance-facebook'] : '';
					if ( isset( $facebook ) && ! empty( $facebook ) ) {
						$social['facebook-f'] = $facebook;
					}
					$google = isset( $option['maintenance-google'] ) ? $option['maintenance-google'] : '';
					if ( isset( $google ) && ! empty( $google ) ) {
						$social['google-plus-g'] = $google;
					}
					$twitter = isset( $option['maintenance-twitter'] ) ? $option['maintenance-twitter'] : '';
