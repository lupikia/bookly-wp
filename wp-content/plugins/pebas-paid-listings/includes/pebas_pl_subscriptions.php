...
/**
 * pebas_pl_subscriptions
 */
class pebas_pl_subscriptions {
	...
	public function filter_job_manager_job_listing_data_fields( $fields, $job_id = false ) {
		...
		if ( isset( $fields['_job_expires'] ) && false !== $subscription_order_id ) {
			...
			$fields['_job_expires']['placeholder'] = __( 'Inherited from subscription', 'pebas-paid-listings' );
			$fields['_job_expires']['information'] = sprintf( __( 'Job listing expires with its <a href="%s">associated subscription</a>.', 'pebas-paid-listings' ), admin_url( 'post.php?post=' . absint( $subscription_order_id ) . '&action=edit' ) );
		...
		if ( 'job_listing' === $post->post_type ) {
			...
			if ( $package_product_id ) {
				...
				if ( 'listing' === $subscription_type ) {
					$new_count = $wpdb->get_var( $wpdb->prepare( "SELECT package_count FROM {$wpdb->prefix}{$table} WHERE id = %d;", $package_id ) );
					$new_count --;
		...
		if ( $id > 0 ) {
			...
			if ( 'job_listing' === $post_type ) {
				...
				if ( $package_product_id ) {
					...
					if ( 'listing' === $subscription_type ) {
						$package  = $wpdb->get_row( $wpdb->prepare( "SELECT package_count, package_limit FROM {$wpdb->prefix}{$table} WHERE id = %d;", $package_id ) );
	...
	public function subscription_ended( $subscription, $paused = false ) {
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			$parent_id         = ! empty( $parent ) ? pebas_pl_get_order_id( $parent ) : null;
			$legacy_id         = isset( $parent_id ) ? $parent_id : pebas_pl_get_order_id( $subscription );
			$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}{$table} WHERE order_id IN ( %d, %d ) AND product_id = %d;", pebas_pl_get_order_id( $subscription ), $legacy_id, $item['product_id'] ) );
			...
			if ( $user_package ) {
				...
				if ( 'listing' === $subscription_type ) {
					...
					foreach ( $listing_ids as $listing_id ) {
						...
						update_post_meta( $listing_id, '_expired_subscription_id', pebas_pl_get_order_id( $subscription ) );
					}
				}
			}
		}// End foreach().

		delete_post_meta( pebas_pl_get_order_id( $subscription ), 'wc_paid_listings_subscription_packages_processed' );
	}
	...
	public function subscription_activated( $subscription ) {
		...
		if ( get_post_meta( pebas_pl_get_order_id( $subscription ), 'wc_paid_listings_subscription_packages_processed', true ) ) {
			return;
		}
		...
		$wpdb->delete( "{$wpdb->prefix}{$table}", array(
			'order_id' => pebas_pl_get_order_id( $subscription ),
		) );
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( $product->is_type( array( 'job_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
				...
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = pebas_pl_give_user_package( $subscription->get_user_id(), $product->get_id(), pebas_pl_get_order_id( $subscription ) );
				}
				 ...
				 */
				if ( 'listing' === $subscription_type ) {
					$listing_ids = (array) $wpdb->get_col( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key=%s AND meta_value=%s", '_expired_subscription_id', pebas_pl_get_order_id( $subscription ) ) );
				} else {
					$listing_ids = array();
				}
				...
				foreach ( $listing_ids as $listing_id ) {
					if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ) ) ) {
						pebas_pl_approve_listing_with_package( $listing_id, $subscription->get_user_id(), $user_package_id );
		...
		update_post_meta( pebas_pl_get_order_id( $subscription ), 'wc_paid_listings_subscription_packages_processed', true );
	}
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( 'package' === $subscription_type ) {
				if ( ! $wpdb->update(
					...
					pebas_pl_give_user_package( $subscription->get_user_id(), $item['product_id'], pebas_pl_get_order_id( $subscription ) );
				}
			} else {
				// Otherwise the listings stay active, but we can ensure they are synced in terms of featured status etc
				if ( $user_package_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}{$table} WHERE order_id IN ( %d, %d ) AND product_id = %d;", pebas_pl_get_order_id( $subscription ), $legacy_id, $item['product_id'] ) ) ) {
					foreach ( $user_package_ids as $user_package_id ) {
						$package = pebas_pl_get_user_package( $user_package_id );

						if ( $listing_ids = pebas_pl_get_listings_for_package( $user_package_id ) ) {
							foreach ( $listing_ids as $listing_id ) {
								// Featured or not
	...
	public function subscription_item_switched( $order, $subscription, $new_order_item_id, $old_order_item_id ) {
		...
		$new_subscription = (object) array(
			'id'           => pebas_pl_get_order_id( $subscription ),
	...
	public function switch_package( $user_id, $new_subscription, $old_subscription ) {
		...
		if ( $user_package ) {
			...
			$switching_to_package_id = pebas_pl_give_user_package( $user_id, $new_subscription->product_id, $new_subscription->id );
				...
				foreach ( $listing_ids as $listing_id ) {
					...
					if ( ! $is_upgrade ) {
						...
						if ( apply_filters( 'job_manager_job_listing_affects_package_count', true, $listing_id ) ) {
							pebas_pl_increase_package_count( $user_id, $switching_to_package_id );
						}
...
pebas_pl_subscriptions::get_instance();
