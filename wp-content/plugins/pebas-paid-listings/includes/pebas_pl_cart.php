...
class pebas_pl_cart {
	...
	public function __construct() {
		...
		add_filter( 'option_woocommerce_enable_signup_and_login_from_checkout', array(
			$this,
	...
	public function legacy_order_item_meta( $item_id, $values ) {
		...
		if ( isset( $values['job_id'] ) ) {
			...
			wc_add_order_item_meta( $item_id, __( 'Job Listing', 'pebas-paid-listings' ), $job->post_title );
	...
	public function checkout_create_order_line_item( $order_item, $cart_item_key, $cart_item_data, $order ) {
		if ( isset( $cart_item_data['job_id'] ) ) {
			...
			$order_item->update_meta_data( __( 'Job Listing', 'pebas-paid-listings' ), $job->post_title );
	...
	public function get_item_data( $data, $cart_item ) {
		if ( isset( $cart_item['job_id'] ) ) {
			...
			if ( isset( $job ) ) {
				$data[] = array(
					'name' => __( 'Job Listing', 'pebas-paid-listings' ),
